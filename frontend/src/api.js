...
const API = {
  ...
  async logout() {
    ...
    if (error) throw new Error(msg(error, "Échec de déconnexion"));
  },
  async me() {
  ...
  // TREES (minimaux pour bosser à deux)
  async getTrees() {
  ...
  async createTree(t) {
    const { error } = await supabase.from("trees").insert({ title: t.title, data: t.data || {} });
    if (error) throw new Error(msg(error));
  },
  async updateTree(id, t) {
    const { error } = await supabase.from("trees").update({ title: t.title, data: t.data || {} }).eq("id", id);
    if (error) throw new Error(msg(error));
  },
  async deleteTree(id) {
  ...
  // Placeholders pour ne rien casser dans l'UI
